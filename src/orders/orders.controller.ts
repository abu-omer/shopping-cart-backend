...
export class OrdersController {
  ...
  async findAll(
    ...
    // Admin users can see all orders; regular customers only see their own.
    if (!user || !user._id) {
      throw new UnauthorizedException('User is not authenticated');
    }
    const userIdFilter = user.role === 'admin' ? undefined : user._id.toString();

    // If a non-admin tries to populate user, and it's not their own user, prevent it.
    if (!userIdFilter && doPopulateUser && user.role !== 'admin') {
      // This scenario won't happen if userIdFilter is correctly applied, but a safety
      // check if the logic changes. For now, `userIdFilter` handles this.
    }
    ...
    return this.ordersService.findAll(
      ...
      page,
      limit,
      doPopulateUser && user.role === 'admin', // Only allow populating user if admin
    );
  }
  @Get('myorders')
